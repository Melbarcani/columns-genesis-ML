...
def get_cells_when_too_close_to_ground(array, board, col, row):
    if col == 1:
        array.extend(package_right_six_cells(board, row, col))
    elif col == COLS_NB - 1:
        array.extend(package_right_six_cells(board, row, col - 3))
    else:
        array.extend(package_right_and_left_cells(board, row, col))
        ...
        for row in range(board.shape[0]):
            for col in range(0, len(board[row])):
                if col != 0 and col != COLS_NB and row != ROWS_NB:
                    ...
                    if col == 4 and row == 0:
                        self.__start = [[row, col], get_column_agent(board, row, col),
                                                 get_nearest_cells(board, row, col)]
                elif row == ROWS_NB:
        ...
        if new_position in self.__states:
            ...
            if supposed_current_position == BORDER:
                ...
                #     reward = REWARD_BORDER - (REWARD_ROW - state[0])  # state[0] == current row
                    ...
                    reward = REWARD_BORDER - (REWARD_ROW - position[0])  # state[0] == current row
                    position = agent.position
            ...
            reward = REWARD_LOSE
        agent.update(get_key(self.__states[position]), action, reward)
        return reward

    def perform_actions(self, action, column, position):
        if action == DOWN:
            new_state = (position[0] + 1, position[1])
        elif action == LEFT:
            new_state = (position[0], position[1] - 1)
        elif action == RIGHT:
            new_state = (position[0], position[1] + 1)
        elif action == CHANGE:
